...
from hfb.strategy import AbstractFrameworkBenchmarkRunner as BaseBenchmarkStrategy
...
logger.add(
    sys.stderr,
    colorize=True,
    format="<green>{time:YY:MM:DD HH:MM}</green> <level>{message}</level>",
)
...
class TestBaseBenchmarkRunner:
    ...
    def test_base_with_additional_param(self):
        ...
        base = TestBenchmarkStrategy(
            server="sanic", p1="v1", p2=True, p3=BaseBenchmarkStrategy
        )
