from hfb.strategy import AbstractFrameworkBenchmarkRunner, AbstractServerRunner
...
class Context:
    def __init__(
        self,
        benchmark_runner: AbstractFrameworkBenchmarkRunner,
        server_runner: AbstractServerRunner,
    ):
...
def get_runner(strategy: str = "ApacheBench", *args, **kwargs):
    if not list(filter(lambda member: strategy == member[0], getmembers(factory))):
        raise NotImplementedError(
            "Missing Benchmark Runner Strategy for: {}".format(strategy)
        )
